...
/**
 * Created by Edwin Syfriana Randy
 */
...
public class ActivityEditMobil extends AppCompatActivity implements InitComponent, View.OnClickListener {
    //declare component
    ...
    public void initToolbar() {
        ...
        getSupportActionBar().setTitle("Update Mobil");
    }
    ...
    public void initValue() {
        ...
        et_nama_mobil.setText(car.getNAMAMOBIL());
        et_merk_mobil.setText(car.getMERKMOBIL());
        et_deskripsi_mobil.setText(car.getDESKRIPSIMOBIL());
        et_tahun.setText(car.getTAHUNMOBIL());
        et_kapasitas.setText(car.getKAPASITASMOBIL());
        et_harga_sewa.setText(car.getHARGAMOBIL());
        et_warna_mobil.setText(car.getWARNAMOBIL());
        sp_bensin.setSelection(Integer.parseInt(car.getBENSINMOBIL())-1);
        et_plat.setText(car.getPLATNOMOBIL());
        statusAktif=Integer.parseInt(car.getSTATUSMOBIL());
        if (car.getSTATUSMOBIL().equals("1"))
            raktif.setChecked(true);
        else
            rtidakaktif.setChecked(true);

        if (car.getIMAGE().size()>0)
            Picasso.with(mContext).load(client.getBaseImg()+"mobil/"+car.getIMAGE().get(0)).into(imgshow);
    ...
    private void register(){
        ...
        register = client.getApi().mobilUpdate(
                car.getIDMOBIL(),
                et_nama_mobil.getText().toString(),
                ...
                if (response.isSuccessful()){
                    if (response.body().getStatus()) {
                        new SweetAlertDialog(mContext, SweetAlertDialog.SUCCESS_TYPE)
                                .setTitleText("Info")
                                .setContentText("Data Mobil Berhasil Di Update!")
                                .show();
                    }else {
                        new SweetAlertDialog(mContext, SweetAlertDialog.ERROR_TYPE)
                                .setTitleText("Info")
                                .setContentText("Data Mobil Gagal Di Update!")
                                .show();
                    }
                }else{
                    new SweetAlertDialog(mContext, SweetAlertDialog.ERROR_TYPE)
